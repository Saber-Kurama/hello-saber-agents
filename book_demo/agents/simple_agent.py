...
class SimpleAgent(Agent):
    """
    MySimpleAgent is a simple agent that can be used to test the HelloAgents framework.
    ...
    def _run_with_tools(self, messages: list[dict[str, str]], input_text: str, max_tool_iterations: int = 3, **kwargs) -> str:    
        """支持多轮工具调用逻辑"""
        current_iteration = 0
        final_response = ""
    ...
    def _execute_tool_call(self, tool_name: str, parameters: str) -> str:
        ...
        try:
            ...
            if tool_name == 'calculator':
                result = self.tool_registry.execute_calculator(tool_name, parameters)
            else:
